...
export const LantumGrid = () => {
  ...
  const [parent, setParent] = useState<number>(8);
  ...
  return (
    <div className="absolute bottom-4 left-4 border border-black/10 rounded-md overflow-hidden shadow-sm dark:border-primary">
      <DndContext
        sensors={[mouseSensor]}
...
const Dropzone = (props: any) => {
  ...
  return (
    <div
      ref={setNodeRef}
      className={cn(
        "h-[90px] border-r border-primary bg-background p-2 border-l-0",
        isOver ? "bg-ui" : "bg-background",
        isOver &&
          active &&
          active.data.current &&
          active.data.current.supports !== props.data &&
          "bg-red-50 ring-2 ring-red-500 dark:bg-red-500/10",
      )}
    >
      {props.children}
    </div>
...
const DraggableGrid = ({ parent, saving, isDragging }: any) => {
  return (
    <div className="grid w-[60vw] max-w-[760px] justify-end grid-cols-7 gap-0 divide-y divide-primary">
      {[...Array(28)].map((_, id) => {
        ...
        return (
          <Dropzone id={id} key={id} data={id < 21 ? "FY1" : "SHO"}>
            ...
            {id % 2 ? (
              <div className="flex flex-col p-1 rounded bg-ui h-10 w-full text-sm"></div>
            ) : null}
          </Dropzone>
        );
      })}
...
const RowHeader = ({ id }: any) => {
  return (
    <div className="h-[90px] p-2 border-r border-primary bg-ui text-xs flex flex-col">
...
// Stolen from Emil Kowalski's Spinner component - animations.dev
const bars = Array(12).fill(0);
const Spinner = ({ color, size = 20 }: any) => {
  return (
    <div
