...
export const LantumBulk = () => {
  ...
  return (
    <div className="relative border border-black/10 rounded-md overflow-hidden shadow-sm dark:border-primary">
      <div className="grid w-[60vw] max-w-[760px] justify-end grid-cols-7 gap-0 divide-y divide-primary">
        {[...Array(28)].map((_, id) => {
          return (
            <div
              key={id}
              className="h-[90px] border-l border-primary bg-background p-2 border-r-0"
            >
  ...
  return (
    <button
      ...
      className={cn(
        ...
        selected && "ring-2 ring-offset-2 ring-offset-ui ring-cyan-500",
      )}
    >
      {children}
    </button>
    ...
    <AnimatePresence>
      {isSelecting ? (
        <motion.div
          ...
          className="absolute left-0 right-0 bottom-4 grid place-items-center"
        >
          <div
            className={cn(
              "bg-ui rounded-xl border border-primary shadow-xl max-w-[420px]",
            )}
          >
            <motion.div
              animate={{
